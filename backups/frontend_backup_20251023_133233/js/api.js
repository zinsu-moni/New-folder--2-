...
class AffluenceAPI {
    ...
    async request(endpoint, options = {}) {
        ...
        const timeoutId = setTimeout(() => {
            controller.abort();
            if (this.debug) console.error([api.js] Request timed out after \ms);
        }, API_CONFIG.TIMEOUT);
        ...
        try {
            ...
            if (err.name === 'AbortError') {
                console.error([api.js] Request timed out after \ms);
    ...
    async adminLogin(email, password) {
        ...
        try {
            ...
            if (this.debug) console.error('[api.js] Admin login error:', err);
            throw err;
        }
    }
        ...
        try {
            ...
            for (const endpoint of testEndpoints) {
                try {
                    ...
                    console.log([api.js] Testing \\: \);
                    if (response.ok) {
                        console.log([api.js] API connection successful via \);
                        return true;
                    }
                } catch (err) {
                    console.log([api.js] Endpoint \ failed: \);
...
function showToast(message, type = 'info') {
    const t = document.createElement('div'); t.className = 	oast toast-\; t.textContent = message;
...
function checkAuth() {
    const publicPages = ['/index.html', '/login.html', '/register.html', '/', '/login-test.html']; // Added login-test.html
